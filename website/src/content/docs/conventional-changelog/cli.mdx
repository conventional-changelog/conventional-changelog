---
title: CLI
description: Command-line usage, flags, and examples for the conventional-changelog CLI.
sidebar:
  order: 2
---
...
The `conventional-changelog` binary reads commits from your git history and the version from your `package.json`, then writes a `CHANGELOG.md`. See [Introduction](/conventional-changelog/) for installation and picking a [preset](/conventional-changelog/#available-presets).
...
<Aside title="Print the list from the CLI">
  Running `conventional-changelog --help` prints the same option list.
</Aside>
...
```sh frame="none"
conventional-changelog -p conventionalcommits --stdout
```
...
```sh frame="none"
conventional-changelog -p conventionalcommits -r 0
```
...
```sh frame="none"
conventional-changelog -p conventionalcommits -r 2 --stdout
```
...
```sh frame="none"
conventional-changelog -p conventionalcommits --from v1.0.0 --to v1.1.0 --stdout
```
...
```sh frame="none"
conventional-changelog -p conventionalcommits -a
```
...
```sh frame="none"
conventional-changelog -p conventionalcommits -u --stdout
```
...
conventional-changelog -p conventionalcommits -t "api-v" --commit-path packages/api --stdout

# Lerna-style tags like "api@1.2.0"
conventional-changelog -p conventionalcommits -l api --commit-path packages/api --stdout
```
...
```sh frame="none"
conventional-changelog -p conventionalcommits -i HISTORY.md
```

Pass `-o` only when the output should go somewhere other than the input:

```sh frame="none"
conventional-changelog -p conventionalcommits -i HISTORY.md -o RELEASES.md
```
...
```sh frame="none"
conventional-changelog -n ./changelog.config.js -c ./context.json --stdout
```
